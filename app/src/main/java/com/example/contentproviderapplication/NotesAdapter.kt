...
class NotesAdapter (private val listener: NoteClickedListener): RecyclerView.Adapter<NotesViewHolder>() {
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, position: Int) {
        ...
        holder.noteTitle.text = mCursor?.getString(mCursor?.getColumnIndexOrThrow(NotesDatabaseHelper.TITLE_NOTES) as Int)
        holder.noteDescription.text = mCursor?.getString(mCursor?.getColumnIndexOrThrow(NotesDatabaseHelper.DESCRIPTION_NOTES) as Int)
